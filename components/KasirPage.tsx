...
export default function KasirPage() {
  ...
  const addToCartWithUnit = (product: Product, unit: 'pack' | 'piece') => {
    ...
    // Cek apakah bungkus masih tersedia jika memilih unit pack
    if (unit === 'pack' && fullPacks <= 0) {
      Swal.fire({
        icon: 'warning',
        title: 'Stok Bungkus Habis!',
        text: 'Tidak ada bungkus utuh tersedia. Silakan beli per eceran.',
        timer: 2500
      });
      return;
    }
  ...
  const updateQuantity = (id: number, unit: 'pack' | 'piece', delta: number) => {
    ...
    const fullPacks = Math.floor(item.stock / piecesPerPack);
    const maxQty = unit === 'piece' 
      ? item.stock
      : fullPacks;
        ...
        >
          <div className="bg-white rounded-2xl max-w-md w-full shadow-2xl animate-in fade-in zoom-in duration-200 max-h-[90vh] overflow-y-auto">
            ...
            <div className="p-3 sm:p-4 space-y-3">
              {(() => {
                const piecesPerPack = Number(selectedProduct.pieces_per_pack) || 1;
                const fullPacks = Math.floor(selectedProduct.stock / piecesPerPack);
                const remainingPieces = selectedProduct.stock % piecesPerPack;
                ...
                return fullPacks <= 0 ? (
                  <div className="bg-amber-50 border border-amber-200 rounded-lg p-2.5 sm:p-3 text-center">
                    <p className="text-[11px] sm:text-xs text-amber-700 leading-relaxed">
                      ⚠️ <strong>{selectedProduct.unit_type || 'Bungkus'}</strong> sudah habis. Hanya tersedia <strong>{remainingPieces} eceran/batang</strong> saja.
                    </p>
                  </div>
                ) : (
                    ...
                    <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-2">
                      <div className="flex-1">
                        ...
                        <p className="text-[11px] sm:text-xs text-slate-600 mt-1">
                          Isi: {piecesPerPack} pcs • Stok: {fullPacks} {selectedProduct.unit_type || 'pack'}{remainingPieces > 0 && ` + ${remainingPieces} pcs`}
                        </p>
                      </div>
                      <div className="text-left sm:text-right">
                        <p className={`font-bold text-base sm:text-lg ${fullPacks <= 0 ? 'text-gray-500' : 'text-indigo-700'}`}>
